        ...
        _In_ sai_ip4_t ip)
{
    ...
    {
        SAI_META_LOG_ERROR("failed to convert IPv4 address, errno: %s", strerror(errno));
        ...
        _In_ const sai_ip_address_t *ip_address)
{
    switch (ip_address->addr_family)
    {
        ...
        default:
            ...
            SAI_META_LOG_ERROR("invalid ip address family: %d", ip_address->addr_family);
        ...
        _In_ const sai_ip_prefix_t *ip_prefix)
{
    switch (ip_prefix->addr_family)
    {
        case SAI_IP_ADDR_FAMILY_IPV4:
            ...
            /* TODO mask */
            sai_serialize_ipv4(buffer, ip_prefix->addr.ip4);

        case SAI_IP_ADDR_FAMILY_IPV6:

            /* TODO mask */
            sai_serialize_ipv6(buffer, &ip_prefix->addr.ip6);

        default:

            SAI_META_LOG_ERROR("invalid ip address family: %d", ip_prefix->addr_family);
        ...
        _In_ bool count_only)
{
    ...
    for (; i < list->count; ++i)
    {
        index += sai_serialize_enum(buffer + index, list->list[i], meta);
